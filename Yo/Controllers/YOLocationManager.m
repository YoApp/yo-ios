...
- (NSString *)checkForKnownPlaceAtCurrentLocation {
    return [self checkForKnownPlaceAtLocation:self.cachedLocation];
}

- (NSString *)checkForKnownPlaceAtLocation:(CLLocation *)location {
    return [self checkForKnownPlaceAtLocation:location resultLocationString:nil];
}

- (NSString *)checkForKnownPlaceAtLocation:(CLLocation *)location resultLocationString:(NSString **)resultLocationString {
    NSDictionary *places = [[NSUserDefaults standardUserDefaults] objectForKey:@"places"];
    NSArray *allKnownLocations = [places allKeys];
    for (NSString *locationString in allKnownLocations) {
        ...
        if (distance >= 0.0 && distance <= 100.0) { // @or: let's try 100 meters
            
            if (resultLocationString) {
                *resultLocationString = locationString;
            }
            
            NSString *placeName = [places objectForKey:locationString];
...
- (void)savePlaceName:(NSString *)name {
    
    // Check to see if this place is already marked (taken)
    NSString *locationString = nil;
    [self checkForKnownPlaceAtLocation:self.cachedLocation resultLocationString:&locationString];
    
    NSMutableDictionary *places = [[[NSUserDefaults standardUserDefaults] objectForKey:@"places"] mutableCopy];
    
    if (places == nil) {
        places = [NSMutableDictionary dictionary];
    }
    else if (locationString != nil) {
        [places removeObjectForKey:locationString];
    }
    
    // allow first part to take place so user can delete location,
    // then check for name length before
    NSCharacterSet *whiteSpaceAndNewLineCharacterSet = [NSCharacterSet whitespaceAndNewlineCharacterSet];
    NSString *trimmedString = [name stringByTrimmingCharactersInSet:whiteSpaceAndNewLineCharacterSet];
    if (trimmedString.length > 0) {
        CLLocation *location = [[YoLocationManager sharedInstance] cachedLocation];
        CLLocationCoordinate2D coordinate = location.coordinate;
        locationString = MakeString(@"%f,%f", coordinate.latitude, coordinate.longitude);
        [places setObject:name forKey:locationString];
    }
...
#pragma mark - CLLocationManager Delegate
             ...
             - (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status
             {
                 if (status == kCLAuthorizationStatusDenied || status == kCLAuthorizationStatusRestricted) {
                     [self completeAllLocationRequests];
                     [[NSNotificationCenter defaultCenter] postNotificationName:YoNotificaitonLocationServicesDenied object:self];
                 }
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_8_0
                         ...
                         [[NSNotificationCenter defaultCenter] postNotificationName:YoNotificaitonLocationServicesAuthorizied object:self];
                         for (YoLocationRequest *locationRequest in self.locationRequests) {
                             [locationRequest startTimeoutTimerIfNeeded];
                         }
                     }
