...
@implementation UIView (YoAdditions)

- (CGFloat)yo_hairlineWidth
{
	CGFloat scale = self.window ? self.window.screen.scale : UIScreen.mainScreen.scale;
	return 1 / scale;
}

- (UIView *)yo_addSeparatorToEdge:(CGRectEdge)edge color:(UIColor *)color
{
	...
	NSDictionary *metrics = @{ @"length": @(self.yo_hairlineWidth) };
	switch (edge) {
		case CGRectMinXEdge:
		case CGRectMaxXEdge: {
