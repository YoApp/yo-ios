...
#import "YoBasicDataSource.h"
#import "YoDataSource+Subclasses.h"

@implementation YoBasicDataSource
...
- (void)updateLoadingStateFromItems
{
    ...
    if (numberOfItems && [loadingState isEqualToString:YoLoadStateNoContent])
        self.loadingState = YoLoadStateContentLoaded;
    else if (!numberOfItems && [loadingState isEqualToString:YoLoadStateContentLoaded])
        self.loadingState = YoLoadStateNoContent;
}
...
{
    return [self.items getObjects:buffer range:range];
}
...
{
    NSUInteger newCount = self.items.count - indexes.count;
