...
@interface YoStateMachine () <YoStateMachineDelegate> {
	OSSpinLock _lock;
	__weak id<YoStateMachineDelegate> _delegate;
...
- (id <YoStateMachineDelegate>)target
{
    id<YoStateMachineDelegate> delegate = self.delegate;
	if (delegate) { return delegate; }
    return self;
}

- (id<YoStateMachineDelegate>)delegate
{
	id<YoStateMachineDelegate> delegate = nil;
...
{
	id<YoStateMachineDelegate> delegate = self.delegate;
	id<YoStateMachineDelegate> target = delegate ?: self;
...
- (void)setDelegate:(id<YoStateMachineDelegate>)delegate
{
	NSParameterAssert([delegate conformsToProtocol:@protocol(YoStateMachineDelegate)]);
...
- (void)setCurrentState:(NSString *)toState
{
	...
	id <YoStateMachineDelegate> target = [self target];
...
- (BOOL)canTransitionToState:(NSString *)state {
    return [self validateTransitionFromState:self.currentState toState:state] != nil;
}
